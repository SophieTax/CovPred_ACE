---
title: "Base Analysis: empirical covariance prediction, 1 test"
...
RUN ANALYSIS: one cell
```{r}
source("base_analysis.R")

analysis1=base_analysis(Y_c_short, X_short_subset, Y_c_conv, X_conv_subset, channel_names, short_ids, conventional_ids)
```

RUN ANALYSIS: all cells
```{r}
source("base_analysis_full.R")

analysis2=base_analysis_full(Y_c_short, X_short_subset, Y_c_conv, X_conv_subset, channel_names, short_ids, conventional_ids)
```
...
list_pvalues
```
LET'S SEE WHAT HAPPENS IF WE USE PREDICTION FORMULA

```{r}
source("prediction_analysis.R")
analysis3=prediction_analysis(Y_c_short, X_short_subset, Y_c_conv, X_conv_subset, channel_names, short_ids, conventional_ids)
```
