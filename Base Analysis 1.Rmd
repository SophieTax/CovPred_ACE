...
```{r}
samplec_short=sample_covariance(Y_c_short,channel_names)
samplec_conv=sample_covariance(Y_c_conv,channel_names)
```
...
PREDICTED COVS FOR SHORT

```{r}
prcov_short=list()
for (i in 1:length(X_short)){
  nn=nn2(X_conv, X_short[i], k=5)
  nn_Ys=samplec_conv[nn$nn.idx]
  new_avg_knn=Reduce("+", nn_Ys)/length(nn_Ys)
  prcov_short[[i]]=new_avg_knn
}
```

PREDICTED COVS FOR CONV

```{r}
prcov_conv=list()
for (i in 1:length(X_conv)){
  nn=nn2(X_short, X_conv[i], k=5)
  nn_Ys=samplec_short[nn$nn.idx]
  new_avg_knn=Reduce("+", nn_Ys)/length(nn_Ys)
  prcov_conv[[i]]=new_avg_knn
}
```

GENERATE Y_C AND Y_T

```{r}
Y_t=append(samplec_short, prcov_conv)
Y_c=append(prcov_short, samplec_conv)
