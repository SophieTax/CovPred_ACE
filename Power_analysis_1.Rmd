...
tmp_a=base_analysis(Y_c_short_new, Y_c_conv_new, channel_names, n_row=2, n_col=1, paired=TRUE)
tmp_b=base_analysis_unbiased(Y_c_short_new, Y_c_conv_new, channel_names, n_row=2, n_col=1, paired=TRUE)
...
pvals2=get_pvalues(analysis_b)
```

```{r}
tibble(no_correction=as.numeric(pvals), with_correction=as.numeric(pvals2))
```
...
for (lambda in seq(1,2,by=0.1)){
  ...
  tmp_c=knn_analysis(X_short, X_conv, Y_c_short_new, Y_c_conv_new, channel_names, n_row=2, n_col=1, K=5)
  tmp_d=knn_analysis(X_short, X_conv, Y_c_short_new, Y_c_conv_new, channel_names, n_row=2, n_col=1, K=5)
  
  analysis_c[[i]]=tmp_c$test
  analysis_d[[i]]=tmp_d$test
  i=i+1
  
}  
pvals=get_pvalues(analysis_c)
pvals2=get_pvalues(analysis_d)
```
