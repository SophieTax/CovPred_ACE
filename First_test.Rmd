...
```
Merge timeseries and subject info, and separate in 2 groups: "short" sleepers and "conventional" sleepers
```{r}
...
Unique X values in short and conventional
```{r}
unique_X_short<-unique(X_short_subj$PSQI_AmtSleep)%>%sort(decreasing = FALSE)
unique_X_conv<-unique(X_conv_subj$PSQI_AmtSleep)%>%sort(decreasing = FALSE)
```

Sampling unseen X values for the 2 groups
```{r}
t_for_short<-data.frame(X=sample(unique_X_conv,size=length(short_ids),replace=TRUE),sub=short_ids)
t_for_conv<-data.frame(X=sample(unique_X_short,size=length(conventional_ids), replace=TRUE),sub=conventional_ids)
...
```{r}
tmp_sample_short_corresp_elem=vector()
tmp_pred_short_corresp_elem=vector()
ttest_short_sleepers=list()
i=1
for (col in 1:(length(channel_names)-1)){
  i=i
  for (row in (col+1):length(channel_names)){
    i=i
    for (s in 1:length(short_ids)){
      tmp_sample_short_corresp_elem[s]=triangle_sample_short$mat[row,col,s]
      tmp_pred_short_corresp_elem[s]=triangle_pred_short$mat[row,col,s]
    }
    test_current=t.test(x=c(tmp_sample_short_corresp_elem), y=c(tmp_pred_short_corresp_elem), paired=TRUE, alternative="two.sided")
    ttest_short_sleepers[[i]]=list()
    ttest_short_sleepers[[i]]=test_current
    i=i+1
  }
}
...
```{r}
tmp_sample_conv_corresp_elem=vector()
tmp_pred_conv_corresp_elem=vector()
ttest_conv_sleepers=list()
i=1
for (col in 1:(length(channel_names)-1)){
  i=i
  for (row in (col+1):length(channel_names)){
    i=i
    for (s in 1:length(conventional_ids)){
      tmp_sample_conv_corresp_elem[s]=triangle_sample_conv$mat[row,col,s]
      tmp_pred_conv_corresp_elem[s]=triangle_pred_conv$mat[row,col,s]
    }
    test_current=t.test(x=c(tmp_sample_conv_corresp_elem), y=c(tmp_pred_conv_corresp_elem), paired=TRUE, alternative="two.sided")
    ttest_conv_sleepers[[i]]=list()
    ttest_conv_sleepers[[i]]=test_current
    i=i+1
  }
}
ttest_conv_sleepers[[2]]
```
Prepare for BH procedure (multiple testing):

pvalues short sleepers
```{r}
pvalues_short=list()
for (i in 1:length(ttest_short_sleepers)){
  pvalues_short[i]=ttest_short_sleepers[[i]][3]
}
```

pvalues conventional sleepers
```{r}
pvalues_conv=list()
for (i in 1:length(ttest_conv_sleepers)){
  pvalues_conv[i]=ttest_conv_sleepers[[i]][3]
}
```
...
```{r}
P_short = matrix(,num_regions,num_regions)
i=1
for (col in 1:(length(channel_names)-1)){
  i=i
  for (row in (col+1):length(channel_names)){
    P_short[row,col]=BH_short[i]
    i=i+1
  }
}
```
...
```{r}
P_conv = matrix(,num_regions,num_regions)
i=1
for (col in 1:(length(channel_names)-1)){
  i=i
  for (row in (col+1):length(channel_names)){
    P_conv[row,col]=BH_conv[i]
    i=i+1
  }
}
```
...
```{r}
plot_short=image(z=P_short)
grid(nx=num_regions, ny=num_regions,col="lightgray")
plot_short
```
```{r}
plot_conv=image(z=P_conv)
grid(nx=num_regions, ny=num_regions,col="lightgray")
plot_conv
```
