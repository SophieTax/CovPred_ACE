...
```{r}
t_1sub<-sample(unique_X_conv,1)
t_for_short<-sample(unique_X_conv,size=241,replace=TRUE)
t_for_conv<-sample(unique_X_short,size=489, replace=TRUE)
```

Try for 1st subject from short sleepers.
```{r}
source("cov_pred_subj.R")
sig_hat_sj1<-cov_pred_subj(fit_cap_C_short$gamma,fit_cap_C_short$beta,t_1sub,gg_inv_s)
```

```{r}
s1_sample_cov<-s_cov_short$YY[1]
```



Next steps:
- make prediction per cov. matrix usando la formula qui sopra. Più in dettaglio, fai prediction sulla matrice di cov. per ogni soggetto usando unseen X values.
