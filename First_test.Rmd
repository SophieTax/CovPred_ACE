...
for (i in 1:length(ids)){
  Y_t_triangle1[[i]]=get_triangle(Y_t[[i]], triangle="LOWER")
}

Y_c_triangle=list()
for (k in 1:length(ids)){
  Y_t_triangle[[k]]=get_triangle(Y_c[[k]], triangle="LOWER")
}
...
```{r}
rownames(pvalues_matrix) = colnames(pvalues_matrix) = channel_names
abs1_limit = pvalues_matrix %>% range
pval_mat_long = melt(pvalues_matrix, na.rm = TRUE)
vec=c(0.05,0.1,0.25)
otherwise = paste0(">",vec[2])
#pval_mat_long$value = factor(pval_mat_long$value,levels = c(otherwise,vec))
p_cov_sub1 = ggplot(data = pval_mat_long, aes(Var2, Var1, fill = value)) +
    geom_tile(color = "white") +
    #scale_fill_gradient2(low = "darkgreen", high = "darkred", mid ="pink", space = "Lab", midpoint=0.2,
                         #name = "Correlation") +
    scale_fill_brewer(name="p-values", palette = "Greens")+
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust = 1),
          axis.title.x=element_blank(),
          axis.title.y=element_blank(),
