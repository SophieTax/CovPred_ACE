...
(For every subject, Yt and Yc are matrices (either sample or predicted))
...
## Comparing
Get triangle matrix, for short sleepers
```{r}
source("get_triangle.R")
triangle_sample_short=list()
for (i in 1:length(short_ids)){
  triangle_sample_short[[i]]=get_triangle(sample_cov_short[[i]])
}

triangle_pred_short=list()
for (k in 1:length(short_ids)){
  triangle_pred_short[[k]]=get_triangle(cov_pred_short[[k]])
}
...
for (i in 1:length(conventional_ids)){
  triangle_sample_conv[[i]]=get_triangle(sample_cov_conventional[[i]])
}

triangle_pred_conv=list()
for (k in 1:length(conventional_ids)){
  triangle_pred_conv[[k]]=get_triangle(cov_pred_conv[[k]])
}

```

```{r}
Y_t_triangle=append(triangle_sample_short,triangle_pred_conv)
Y_c_triangle=append(triangle_pred_short,triangle_sample_conv)
Y_diff_triangle=list()
for (m in 1:length(ids)){
  Y_diff_triangle[[m]]=Y_t_triangle[[m]]-Y_c_triangle[[m]]
}
```
```{r}
...
```{r}
plot_short=image(x=(0:num_regions), y=(0:num_regions),z=BH_matrix, xlab="regions", ylab="regions")
grid(nx=num_regions, ny=num_regions,col="lightgray")
#plot_short
library(lattice)
library(RColorBrewer)
levelplot(BH_matrix, xlab="regions", ylab="regions", col.regions=brewer.pal(6,"PuRd"))
```
```{r}
