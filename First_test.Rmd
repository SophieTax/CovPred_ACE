...
```{r}
ttest1<-t.test(x=c(triangle_sample_short$mat[,,1]), y=c(triangle_sample_conv$mat[,,1]), paired = TRUE, alternative = "two.sided")
ttest1
```
...
tmp_short=append(tmp_ssample,tmp_spred)
```



WAIT FOR RESPONSE FROM CHRISTOF TO IMPLEMENT NEW TTEST
```{r}
tmp_psample=vector()
for (s in 1:length(conventional_ids)){
  tmp_psample[s]=triangle_sample_conv$mat[2,1,s]
}

tmp_ppred=vector()
for (s in 1:length(conventional_ids)){
  tmp_ppred[s]=triangle_pred_conv$mat[2,1,s]
}

tmp_conventional=append(tmp_psample,tmp_ppred)
```

t_test on those elements
```{r}
ttest_pairwise_1el=t.test(x=c(tmp_short), y=c(tmp_conventional),paired=TRUE, alternative = "two.sided")
```
...
levelplot(P_short, xlab="regions", ylab="regions", col.regions=brewer.pal(6,"PuRd"))
```
```{r}
