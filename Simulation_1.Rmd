...
#  t_for_conv_new<-sample_unseen(X_for_conv$X,conventional_ids)
  ...
  #cov_pred_short_new = cov_pred(fit_short$gamma,fit_short$beta,X_for_short[,2],gg_inv_s_new)
  #cov_pred_conv_new = cov_pred(fit_conv$gamma,fit_conv$beta,X_for_conv,gg_inv_c_new)
  
  cov_pred_short_new <- replicate(length(short_ids), avg_cov_treatment, simplify=FALSE)
  cov_pred_conv_new <- replicate(length(conventional_ids), avg_cov_control, simplify=FALSE)
